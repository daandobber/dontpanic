...
static void open_dataset(app_state_t *st, size_t index) {
    close_dataset(st);
...
static void open_article(app_state_t *st, const wiki_result_t *result) {
    ...
    float content_h          = st->h - GUIDE_HEADER_H - GUIDE_FOOTER_H - 16;
...
static void render_datasets(app_state_t *st) {
    ...
    theme_draw_footer(st->fb, st->w, st->h, "UP/DOWN select   ENTER open   ESC/F1 exit");
    theme_draw_scanlines(st->fb, st->w, st->h);
}
...
static void render_browser(app_state_t *st) {
    ...
    float y = GUIDE_HEADER_H + 8;
    char  search_line[80];
    snprintf(search_line, sizeof(search_line), "LOOK UP SUBJECT> %.48s_", st->search_buf);
    pax_draw_text(st->fb, GUIDE_COLOR_AMBER, GUIDE_FONT, LIST_FONT_SIZE, GUIDE_MARGIN, y, search_line);
    y += LIST_ROW_HEIGHT + 4;

    if (st->result_count == 0) {
        pax_draw_text(st->fb, GUIDE_COLOR_GREEN_DIM, GUIDE_FONT, LIST_FONT_SIZE, GUIDE_MARGIN, y,
                      "-- no matching entries --");
    } else {
        for (size_t i = 0; i < st->result_count; i++) {
            bool selected = (i == st->result_selected);
            if (selected) {
                pax_simple_rect(st->fb, GUIDE_COLOR_GREEN_DIM, GUIDE_MARGIN, y, st->w - 2 * GUIDE_MARGIN,
                                LIST_ROW_HEIGHT);
            }
            char line[128];
        ...
        if (st->result_has_more) {
            pax_draw_text(st->fb, GUIDE_COLOR_CYAN, GUIDE_FONT, 14, GUIDE_MARGIN, y + 2, "v more entries below v");
        }
    }

    theme_draw_footer(st->fb, st->w, st->h, "TYPE to search   ENTER open   ESC back");
    theme_draw_scanlines(st->fb, st->w, st->h);
}
...
static void render_home(app_state_t *st) {
    theme_clear(st->fb);
    theme_draw_panel(st->fb, 3, 3, st->w - 6, st->h - 6, GUIDE_COLOR_GREEN_DIM);
    theme_draw_header(st->fb, st->w, st->current_dataset.info.name);
    ...
    pax_draw_text(st->fb, GUIDE_COLOR_CYAN, GUIDE_FONT, 14, GUIDE_MARGIN, st->h - GUIDE_FOOTER_H - 22,
                  "F4 in an article saves it as favorite. Categories are not indexed yet.");
    theme_draw_footer(st->fb, st->w, st->h, "UP/DOWN select   ENTER open   ESC datasets");
    theme_draw_scanlines(st->fb, st->w, st->h);
}
...
static void render_reader(app_state_t *st) {
    ...
    theme_draw_header(st->fb, st->w, safe_title(&st->current_result, title_buf, sizeof(title_buf)));
        ...
        for (size_t i = st->reader_scroll; i < end; i++) {
            ...
            if (st->article.link_count > 0 && st->reader_link_selected < st->article.link_count) {
                ...
                if (ranges_overlap(line_start, line_end, link->text_start, link->text_end)) {
                    ...
                    float glyph_w    = theme_glyph_width(BODY_FONT_SIZE);
                    highlight_x      = GUIDE_MARGIN + (float)(sel_start - line_start) * glyph_w;
                    highlight_w      = (float)(sel_end - sel_start) * glyph_w;
            ...
            copy_ascii_safe(buf, sizeof(buf), st->article.text + ln->offset, n);
            if (highlight) {
    ...
    if (st->reader_status[0]) {
        pax_draw_text(st->fb, GUIDE_COLOR_AMBER, GUIDE_FONT, 14, GUIDE_MARGIN, st->h - GUIDE_FOOTER_H - 38,
                      st->reader_status);
    }
    ...
    char hint[128];
    if (st->article.link_count > 0) {
        snprintf(hint, sizeof(hint), "UP/DN scroll  L/R link %u/%u  ENTER open  ESC back  [%u/%u]",
                 (unsigned)(st->reader_link_selected + 1), (unsigned)st->article.link_count, (unsigned)shown_last,
                 (unsigned)total);
    } else {
        snprintf(hint, sizeof(hint), "UP/DN scroll  PGUP/PGDN page  ESC back  [%u/%u]", (unsigned)shown_last,
                 (unsigned)total);
    }
    theme_draw_footer(st->fb, st->w, st->h, hint);
    theme_draw_scanlines(st->fb, st->w, st->h);
}
